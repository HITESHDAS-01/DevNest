...
async function main() {
  ...
  const taskData = [
    // TaskFlow
    { projectId: projects[0].id, title: 'Design database schema', status: 'done' as const, priority: 1, order: 1, tags: ['backend'] },
    { projectId: projects[0].id, title: 'Build task CRUD API', status: 'done' as const, priority: 1, order: 2, tags: ['backend'] },
    { projectId: projects[0].id, title: 'Create task list UI', status: 'in_progress' as const, priority: 1, order: 3, tags: ['frontend'] },
    { projectId: projects[0].id, title: 'Add drag-and-drop reordering', status: 'todo' as const, priority: 2, order: 4, tags: ['frontend'] },
    { projectId: projects[0].id, title: 'Write integration tests', status: 'backlog' as const, priority: 3, order: 5, tags: ['testing'] },

    // DevNest
    { projectId: projects[1].id, title: 'Set up Drizzle + Postgres', status: 'done' as const, priority: 1, order: 1, tags: ['backend', 'infra'] },
    { projectId: projects[1].id, title: 'Build org/project dashboard', status: 'in_progress' as const, priority: 1, order: 2, tags: ['frontend'] },
    { projectId: projects[1].id, title: 'Implement auth flow', status: 'todo' as const, priority: 1, order: 3, tags: ['auth'] },
    { projectId: projects[1].id, title: 'Create blocker/notes modules', status: 'todo' as const, priority: 2, order: 4, tags: ['frontend'] },

    // SiteGen
    { projectId: projects[2].id, title: 'Research visual editor options', status: 'done' as const, priority: 1, order: 1, tags: ['research'] },
    { projectId: projects[2].id, title: 'Prototype page builder UI', status: 'in_progress' as const, priority: 1, order: 2, tags: ['frontend'] },
    { projectId: projects[2].id, title: 'Define template schema', status: 'todo' as const, priority: 2, order: 3, tags: ['design'] },

    // APIProxy
    { projectId: projects[3].id, title: 'Implement rate limiter', status: 'done' as const, priority: 1, order: 1, tags: ['backend'] },
    { projectId: projects[3].id, title: 'Add response caching', status: 'done' as const, priority: 1, order: 2, tags: ['backend'] },
    { projectId: projects[3].id, title: 'Fix memory leak under load', status: 'todo' as const, priority: 1, order: 3, tags: ['bug', 'critical'] },
    { projectId: projects[3].id, title: 'Load testing with k6', status: 'in_progress' as const, priority: 2, order: 4, tags: ['testing'] },
  ];
  ...
  // ── Blockers ──
  const blockerData = [
    { projectId: projects[1].id, title: 'Auth provider SDK has breaking changes', severity: 'high' as const, status: 'open' as const, description: 'Waiting on vendor to release patch.' },
    { projectId: projects[3].id, title: 'Memory leak in Go proxy goroutines', severity: 'critical' as const, status: 'open' as const, description: 'Heap profile shows uncleared request contexts.' },
    { projectId: projects[0].id, title: 'Drag-and-drop library lacks touch support', severity: 'medium' as const, status: 'open' as const, description: 'Need mobile-friendly alternative.' },
  ];
  ...
  // ── Milestones ──
  const milestoneData = [
    { projectId: projects[0].id, name: 'v0.1 Alpha', targetDate: '2026-03-01', status: 'completed' as const, order: 1 },
    { projectId: projects[0].id, name: 'v0.5 Beta', targetDate: '2026-03-20', status: 'active' as const, order: 2 },
    { projectId: projects[1].id, name: 'MVP Launch', targetDate: '2026-05-01', status: 'active' as const, order: 1 },
    { projectId: projects[2].id, name: 'Prototype Ready', targetDate: '2026-04-15', status: 'pending' as const, order: 1 },
  ];
